...
PROJDIR <- '/data/CARD_AUX/users/wellerca/PFC_data_upload'
...
for(i in unique(dat$sample)) {
    ss <- dat[sample==i]
    ss[modality=='snRNA', 'library_type' := 'Chromatin Accessibility']
    ss[modality=='snATAC', 'library_type' := 'Gene Expression']
